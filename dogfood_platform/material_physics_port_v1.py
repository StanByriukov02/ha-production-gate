...
from dogfood_platform.open_seed_paths_v1 import materials_registry_path
...
def _heal_material_bind() -> dict[str, Any]:
    """Restore empty/corrupt bind from honesty_latch mirror (dogfood self-heal)."""
    mirror = (
        _REPO
        / "open_surface"
        / "honesty_latch"
...
def resolve_tread_profile(profile_id: str) -> dict[str, Any]:
    from dogfood_platform.robot_material_catalog_v1 import resolve_tread_variant

    return resolve_tread_variant(profile_id)


def _seal_scale_for_class(seal_class: str) -> tuple[float, str]:
    from dogfood_platform.robot_material_catalog_v1 import resolve_seal_scale
...
def _wheel_geometry_for_variant(registry: dict[str, Any], variant: dict[str, Any]) -> tuple[float, float, float]:
    from dogfood_platform.robot_material_catalog_v1 import resolve_wheel_geometry
...
def compute_env_in_from_material(
    *,
    ...
    from dogfood_platform.robot_material_catalog_v1 import (
