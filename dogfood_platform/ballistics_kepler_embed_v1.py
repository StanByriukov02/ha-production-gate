...
def _hostile_v_mult() -> float:
    from dogfood_platform.rigid_hop_on_v1 import load_rigid_hop_catalog
...
def _visviva_anchors() -> tuple[float, float, str]:
    from dogfood_platform.orbital_visviva_on_v1 import load_orbital_catalog
...
def _peer_kepler_metric(*, condition: ConditionId, leo_r: float, geo_r: float, body: str) -> float:
    from dogfood_platform.orbital_visviva_on_v1 import evaluate_orbital_visviva
...
def evaluate_ballistics_kepler(
    *,
    ...
    from dogfood_platform.orbital_visviva_on_v1 import evaluate_orbital_visviva
    from dogfood_platform.rigid_hop_on_v1 import evaluate_rigid_hop
