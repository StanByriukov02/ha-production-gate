...
from dogfood_platform.robot_os_policy_stub_v1 import DEFAULT_INJECT_BAD_AT, propose as stub_propose
...
class RegolithPlannerBackend:
    ...
    def propose(self, state: dict[str, Any], carrier_id: str) -> PolicyProposal:
        from dogfood_platform.robot_os_policy_stub_v1 import next_action_id
...
def resolve_policy_port(state: dict[str, Any]) -> PolicyPort:
    ...
    if backend == BACKEND_SMOLVLA_TRACE:
        from dogfood_platform.robot_os_policy_smolvla_trace_v1 import (
            SmolVlaTraceBackend,
            load_smolvla_trace,
        )
