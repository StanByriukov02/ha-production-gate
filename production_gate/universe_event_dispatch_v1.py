...
from production_gate.lunar_dust_ingress_v1 import electrostatic_index, ingress_rate_g_m2_per_sol
from production_gate.lunar_micrometeoroid_threat_v1 import threat_mass_in_band
from production_gate.lunar_thermal_l5_v1 import radiative_net_flux_w_m2
from production_gate.arena.eds_2r_sense_v1 import sense_delta_c
from production_gate.arena.eds_2r_transport_v1 import compare_idle_vs_tw, simulate_transport_tw
from production_gate.arena.maxwell_g1_v1 import compare_g0_vs_g1
from production_gate.chamber.chamber_envelope_v1 import pump_down_profile
from production_gate.lunar_regolith_thermal_v1 import effective_k_w_mk
from production_gate.lunar_site_burial_v1 import burial_thickness_m
from production_gate.lunar_lc2_duty_bind_v1 import evaluate_lc2_duty_bind
from production_gate.lunar_radiation_shield_v1 import classify_regolith_shield
from production_gate.universe_env_radiation_v1 import radiation_fet_from_env, radiation_state_delta
from production_gate.arena.arena_moon_coupling_v1 import compare_coupled_vs_baseline, moon_tile_environment
...
def _dispatch_universe_corridor(params: dict[str, Any], event: dict[str, Any]) -> dict[str, Any]:
    ...
    mod = importlib.import_module("production_gate.universe_kernel_v1")
...
def _dispatch_regolith_robot(params: dict[str, Any], event: dict[str, Any]) -> dict[str, Any]:
    mod = importlib.import_module("production_gate.world_regolith_robot_v0")
    n = float(params.get("n_sols") or 30.0)
...
def _load_kinematic_spike(*, full: bool = False) -> dict[str, Any]:
    ...
    if full:
        from production_gate.universe_backend_native_v1 import run_kinematic_spike_native
...
def _dispatch_kinematic_shock_full(params: dict[str, Any], event: dict[str, Any]) -> dict[str, Any]:
    from production_gate.universe_backend_native_v1 import run_kinematic_event_full
...
def _load_operator_rg_spike(*, full: bool = False) -> dict[str, Any]:
    ...
    if full:
        from production_gate.universe_backend_native_v1 import run_operator_rg_spike_native
...
def _dispatch_ascent_acoustic(params: dict[str, Any], event: dict[str, Any]) -> dict[str, Any]:
    from production_gate.rocket_ascent_load_v1 import evaluate_ascent_envelope
...
def macro_cosmic_dispatch_audit(catalog: dict[str, Any] | None = None) -> dict[str, Any]:
    from production_gate.universe_event_sampler_v1 import load_event_catalog, draw_event_params
    import random
