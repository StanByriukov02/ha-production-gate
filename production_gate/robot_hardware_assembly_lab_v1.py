"""Robot hardware assembly lab v1 — Earth · Moon · Orbit engineer workflow.
...
_LAB = _REPO / "fixtures" / "robot" / "robot_hardware_assembly_lab_v1.json"
_CANON = "docs/agent_workflow/ROBOT_HARDWARE_ASSEMBLY_LAB_V1.md"
...
def run_earth_bench_carrier_recipe(*, write_manifest: bool = False) -> dict[str, Any]:
    from production_gate.appendage_robot_create_cli_v1 import run_robot_create_smoke
    from production_gate.rolling_kinematics_crown_v1 import run_rolling_kinematics_crown_smoke
    from production_gate.wheeled_chassis_compose_v1 import run_wheeled_chassis_smoke
    ...
    from production_gate.physics_measured_bench_ingress_v1 import ingest_earth_lab_bench_for_recipe
...
def run_lunar_scout_field_recipe() -> dict[str, Any]:
    ...
    from production_gate.full_body_compose_v1 import run_full_body_compose_smoke
    from production_gate.hexapod_foot_contact_v1 import run_hexapod_foot_contact_smoke
    from production_gate.kinematic_chain_ir_v1 import clear_chain_overlay
    from production_gate.robot_os_newton_x_world_step_v1 import init_newton_x_world, step_newton_x_world, validate_newton_x_falsifiers
...
def run_orbit_thermal_bus_recipe() -> dict[str, Any]:
    from production_gate.physics_universe_environment_register_v1 import get_world
...
def run_robot_hardware_assembly_lab_smoke(*, recipe_id: str | None = None) -> dict[str, Any]:
    lab = load_assembly_lab_spec()
    ...
    fail = [k for k, v in checks.items() if not v]
    return {
        "verdict": "ROBOT_HARDWARE_ASSEMBLY_LAB_SLICE_PASS" if not fail else "ROBOT_HARDWARE_ASSEMBLY_LAB_SLICE_FAIL",
...
if __name__ == "__main__":
    ...
    print(json.dumps(run_robot_hardware_assembly_lab_smoke(recipe_id=rid), indent=2))
