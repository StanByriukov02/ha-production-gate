...
def validate_region_honesty(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    for reg in regions:
        ...
        if reg.get("chain_id"):
            from production_gate.kinematic_chain_ir_v1 import resolve_chain_spec
            from production_gate.torso_spine_chain_v1 import register_torso_spine_chain
            from production_gate.head_neck_pan_tilt_v1 import register_head_neck_chain
            from production_gate.gripper_subchain_v1 import register_gripper_chain
...
def run_full_body_compose_smoke() -> dict[str, Any]:
    from production_gate.appendage_dual_review_v1 import run_appendage_dual_review
    from production_gate.appendage_motion_bus_v1 import load_motion_bus_spec, robot_tick
