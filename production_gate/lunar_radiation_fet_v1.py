...
from production_gate.lunar_radiation_proxy_v1 import RadiationProxyResult, evaluate_coupled_radiation_proxy

from production_gate.open_seed_paths_v1 import radiation_fet_coeff_bind_path
    ...
    if isinstance(rad, dict) and "albedo_fraction" in rad:
        if "mission_dose_gy" not in rad and "total_dose_gy" in rad:
            ...
            if "see_rate_per_year" not in out:
                from production_gate.lunar_albedo_dose_v1 import evaluate_coupled_surface_dose
...
def _base_see_rate() -> float:
    ...
    from production_gate.lunar_albedo_dose_v1 import load_albedo_dose_bind
