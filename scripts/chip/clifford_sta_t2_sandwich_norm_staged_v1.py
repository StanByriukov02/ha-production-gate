"""T2.12 — sandwich norm partial staging @ EX3 + STA delta vs T2.10."""
...
_RECEIPT = _CHIP / "CHIP_CLIFFORD_STA_T2_SANDWICH_NORM_STAGED_RECEIPT_v1.json"
...
def _sim_dual_tb() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_sandwich_norm_vlt_") as tmp:
        ...
        try:
            ...
            return {
                "verdict": "IVERILOG_SANDWICH_NORM_STAGED_PASS" if ok else "IVERILOG_SANDWICH_NORM_STAGED_FAIL",
                "stdout_tail": out[-1200:],
            }
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as exc:
            return {"verdict": "IVERILOG_SANDWICH_NORM_STAGED_FAIL", "reason": str(exc)[-400:]}


def run_sta_t2_sandwich_norm_staged(*, write: bool = True) -> dict[str, Any]:
    ...
    norm_staged = (
        "lat_norm_acc" in sw
        and "u_acc_low_synth" in sw
        and "u_norm_tail_synth" in sw
        and "lat_norm_acc_real" in sw
    )
    sim = _sim_dual_tb()
    ...
    binding = "sandwich_norm_staged" if "u_norm_tail_synth" in tail or norm_staged else "sandwich_norm_comb"

    checks = [
        {"id": "t2_10_prerequisite", "pass": prior_ok},
        {"id": "binding_path_doc", "pass": _SCOPE.is_file()},
        {"id": "sandwich_norm_staged_rtl", "pass": norm_staged},
        {"id": "iverilog_sandwich_dual_9case", "pass": sim.get("verdict") == "IVERILOG_SANDWICH_NORM_STAGED_PASS"},
        {"id": "opensta_liberty_ran", "pass": sta.get("opensta_run") and sta.get("checks_ok")},
        {
            ...
            "detail": f"prior={prior} norm={wns} delta={delta_prior}",
        },
        {"id": "timing_not_closed", "pass": wns is not None and wns < 0},
    ]

    timing_closed = wns is not None and wns >= 0
    verdict = "STA_T2_SANDWICH_NORM_STAGED_PASS" if all(c["pass"] for c in checks) else "STA_T2_SANDWICH_NORM_STAGED_FAIL"
    receipt: dict[str, Any] = {
        "receipt_id": "CHIP_CLIFFORD_STA_T2_SANDWICH_NORM_STAGED_RECEIPT_v1",
        "verdict": verdict,
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "checks": checks,
        "timing": {
            "t2_10_prior_wns_ns": prior,
            "norm_staged_wns_ns": wns,
        ...
        "honesty": {
            ...
            "staged_split": "acc_low @ ex3_eval · norm_tail @ ex3_latch — REVERTED",
            "t2_12_falsifier": "acc_low comb on ara_mux deepens ex3_eval; tail still binds @ ex3_latch",
            "rtl_reverted_to": "T2.10 gp1 blades + full norm @ ex3_latch",
...
if __name__ == "__main__":
    print(json.dumps(run_sta_t2_sandwich_norm_staged(), indent=2))
