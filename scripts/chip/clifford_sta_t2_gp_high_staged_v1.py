"""T2.19 — geo_prod high @ ex3_latch · pipe_r concat @ wb_eval."""
...
_RECEIPT = _CHIP / "CHIP_CLIFFORD_STA_T2_GP_HIGH_STAGED_RECEIPT_v1.json"
_PRIOR = _CHIP / "CHIP_CLIFFORD_STA_T2_SQRT_I1_SPLIT_RECEIPT_v1.json"
_SCOPE = _REPO / "docs/agent_workflow/CLIFFORD_STA_SANDWICH_BINDING_PATH_v1.md"
_MSYS_BIN = Path(r"C:\msys64") / "mingw64" / "bin"
_WNS_IMPROVE_FLOOR_NS = 0.03
...
def _sim_iverilog(rtl: tuple[str, ...], pass_token: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_t2_gp_high_vlt_") as tmp:
...
def run_sta_t2_gp_high_staged(*, write: bool = True) -> dict[str, Any]:
    ...
    staged_rtl = (
        ...
        and "lat_high <= comb_high_mux" in gp
        and "if (wb_eval)" in gp
        and "pipe_r <= {lat_high, lat_low}" in gp
        and ".wb_eval(phi_wb_eval)" in top
    )
    ...
    binding = "gp_high_staged" if "u_geo_prod_pipe" in tail or "u_high_synth" in tail else "unknown"

    checks = [
        {"id": "t2_18_prerequisite", "pass": prior_ok},
        {"id": "binding_path_doc", "pass": _SCOPE.is_file()},
        {"id": "gp_high_staged_rtl", "pass": staged_rtl},
        ...
        {
            "id": "wns_improved_vs_t2_18",
            "pass": delta_prior is not None and delta_prior >= _WNS_IMPROVE_FLOOR_NS,
            "detail": f"prior={prior} gp_high={wns} delta={delta_prior}",
        },
        {"id": "timing_not_closed", "pass": wns is not None and wns < 0},
    ]
    promote = all(c["pass"] for c in checks if c["id"] != "timing_not_closed")
    verdict = "STA_T2_GP_HIGH_STAGED_PASS" if promote else "STA_T2_GP_HIGH_STAGED_FAIL"

    receipt: dict[str, Any] = {
        "schema": "CHIP_CLIFFORD_STA_T2_GP_HIGH_STAGED_RECEIPT_v1",
        ...
        "timing": {
            "wns_ns": wns,
            "prior_t2_18_wns_ns": prior,
            "delta_vs_t2_18_ns": delta_prior,
            "period_ns": 10.0,
        },
...
if __name__ == "__main__":
    print(json.dumps(run_sta_t2_gp_high_staged(), indent=2))
