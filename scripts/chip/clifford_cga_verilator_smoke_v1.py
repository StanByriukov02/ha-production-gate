"""Verilator iron smoke — CGA motor geo_prod fork (T5)."""
...
_SOURCES = (
    ...
    "clifford_geo_prod_cga_tb_v0.v",
)


def run_cga_verilator_smoke(*, timeout: int = 600) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_cga_vlt_") as tmp:
        build_posix = posix_path(Path(tmp) / "build")
        srcs = " ".join(_SOURCES)
        cmd = (
            f"cd '{fix_posix}' && mkdir -p '{build_posix}' && "
            f"verilator --binary --top-module clifford_geo_prod_cga_tb_v0 "
            f"-Wall -Wno-fatal -CFLAGS '-D_GLIBCXX_USE_CXX11_ABI=0' -LDFLAGS '-lstdc++' "
            f"-I. -Mdir '{build_posix}' {srcs} && "
            f"'{build_posix}/Vclifford_geo_prod_cga_tb_v0.exe'"
        )
        try:
            proc = run_mingw_shell(cmd, timeout=timeout)
            out = (proc.stdout or "") + (proc.stderr or "")
            ok = proc.returncode == 0 and "TB_PASS cga_motor_smoke" in out
            return {
                "verdict": "VERILATOR_CGA_PASS" if ok else "VERILATOR_CGA_FAIL",
                "returncode": proc.returncode,
                "stdout_tail": out[-1500:],
            }
        except Exception as exc:
            return {"verdict": "VERILATOR_CGA_FAIL", "reason": str(exc)[-400:]}
