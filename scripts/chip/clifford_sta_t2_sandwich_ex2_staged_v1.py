"""T2.9 — sandwich EX2 staged promote (rev @ ex2_eval · gp1 @ ex2_latch) + STA delta."""
...
_RECEIPT = _CHIP / "CHIP_CLIFFORD_STA_T2_SANDWICH_EX2_STAGED_RECEIPT_v1.json"
_PRIOR = _CHIP / "CHIP_CLIFFORD_STA_T2_SANDWICH_STAGED_PROMOTE_RECEIPT_v1.json"
...
def _sim_dual_tb() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_sandwich_ex2_vlt_") as tmp:
        ...
        try:
            ...
            return {
                "verdict": "IVERILOG_SANDWICH_EX2_STAGED_PASS" if ok else "IVERILOG_SANDWICH_EX2_STAGED_FAIL",
                "stdout_tail": out[-1200:],
            }
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as exc:
            return {"verdict": "IVERILOG_SANDWICH_EX2_STAGED_FAIL", "reason": str(exc)[-400:]}


def _prior_wns() -> float | None:
    if _PRIOR.is_file():
        w = json.loads(_PRIOR.read_text(encoding="utf-8")).get("timing", {}).get("staged_wns_ns")
        if w is not None:
            return float(w)
    if _THERM.is_file():
...
def run_sta_t2_sandwich_ex2_staged(*, write: bool = True) -> dict[str, Any]:
    ...
    prior_ok = _PRIOR.is_file() and json.loads(_PRIOR.read_text(encoding="utf-8")).get("verdict") == "STA_T2_SANDWICH_STAGED_PROMOTE_PASS"
    sw = (_FIX / "clifford_sandwich_ex_pipe_v0.v").read_text(encoding="utf-8")
    ex2_staged = (
        "lat_ab_low" in sw
        and "lat_ab_high" in sw
        and "u_ab_low_synth" in sw
        and "if (ex2_eval)" in sw
        and "if (ex2_latch)" in sw
    )
    sim = _sim_dual_tb()
    prior = _prior_wns()
    ...
    checks = [
        {"id": "t2_8_prerequisite", "pass": prior_ok},
        {"id": "binding_path_doc", "pass": _SCOPE.is_file()},
        {"id": "sandwich_ex2_staged_rtl", "pass": ex2_staged},
        {"id": "iverilog_sandwich_dual_9case", "pass": sim.get("verdict") == "IVERILOG_SANDWICH_EX2_STAGED_PASS"},
        {"id": "opensta_liberty_ran", "pass": sta.get("opensta_run") and sta.get("checks_ok")},
        {
            "id": "wns_improved_vs_t2_8",
            "pass": delta_prior is not None and delta_prior >= _WNS_IMPROVE_FLOOR_NS,
            "detail": f"prior={prior} ex2={wns} delta={delta_prior}",
        },
        {
    ...
    verdict = "STA_T2_SANDWICH_EX2_STAGED_PASS" if all(c["pass"] for c in checks) else "STA_T2_SANDWICH_EX2_STAGED_FAIL"
    receipt: dict[str, Any] = {
        "receipt_id": "CHIP_CLIFFORD_STA_T2_SANDWICH_EX2_STAGED_RECEIPT_v1",
        "verdict": verdict,
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "checks": checks,
        "timing": {
            "t2_8_prior_wns_ns": prior,
            "ex2_staged_wns_ns": wns,
            "wns_delta_vs_t2_8_ns": delta_prior,
        ...
        "honesty": {
            ...
            "staged_split": "gp1 low/high blades @ ex2 · gp2 @ ex3_eval · norm @ ex3_latch",
...
if __name__ == "__main__":
    print(json.dumps(run_sta_t2_sandwich_ex2_staged(), indent=2))
