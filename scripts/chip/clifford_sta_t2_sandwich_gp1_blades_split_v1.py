"""T2.22 — sandwich gp1 quad blade split @ EX2: lo/hi 0+1/2+3 + 4+5/6+7 · gp2 coarse."""
...
_RECEIPT = _CHIP / "CHIP_CLIFFORD_STA_T2_SANDWICH_GP1_BLADES_SPLIT_RECEIPT_v1.json"
_PRIOR = _CHIP / "CHIP_CLIFFORD_STA_T2_GP_LOW_BLADES_SPLIT_RECEIPT_v1.json"
_SCOPE = _REPO / "docs/agent_workflow/CLIFFORD_STA_SANDWICH_BINDING_PATH_v1.md"
_MSYS_BIN = Path(r"C:\msys64") / "mingw64" / "bin"
_WNS_IMPROVE_FLOOR_NS = 0.3
...
def _sim_dual_tb() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_t2_sw_gp1_vlt_") as tmp:
        out_vvp = Path(tmp) / "sim.vvp"
        try:
            subprocess.run(
                [str(iverilog), "-g2012", "-o", str(out_vvp), "-I", str(_FIX), *[_FIX / s for s in _SW_RTL]],
                check=True,
                capture_output=True,
                text=True,
                timeout=300,
...
def run_sta_t2_sandwich_gp1_blades_split(*, write: bool = True) -> dict[str, Any]:
    ...
    staged_rtl = (
        "T2.22" in sw
        and "lat_ab_low_lo" in sw
        and "u_ab_low_lo_synth" in sw
        and "u_ab_high_hi_synth" in sw
        and "if (ex2_eval)" in sw
        and "lat_ab_high_lo" in sw
        and "if (ex2_latch)" in sw
        and "{ab_high_hi_mux, lat_ab_high_lo, ab_low_hi_mux, lat_ab_low_lo}" in sw
        and "u_ara_low_synth" in sw
    )
    sim_dual = _sim_dual_tb()
    ...
    binding = (
        "sandwich_gp1_high_hi"
        if "u_ab_high_hi_synth" in tail
        else ("sandwich_gp1" if "u_ab_" in tail and "u_sandwich" in tail else "unknown")
    )

    checks = [
        {"id": "t2_21_prerequisite", "pass": prior_ok},
        {"id": "binding_path_doc", "pass": _SCOPE.is_file()},
        {"id": "sandwich_gp1_quad_split_rtl", "pass": staged_rtl},
        {"id": "iverilog_sandwich_dual", "pass": sim_dual.get("verdict") == "IVERILOG_PASS"},
        ...
        {
            "id": "wns_improved_vs_t2_21",
            "pass": delta_prior is not None and delta_prior >= _WNS_IMPROVE_FLOOR_NS,
            "detail": f"prior={prior} sw_gp1={wns} delta={delta_prior}",
        },
        {"id": "timing_not_closed", "pass": wns is not None and wns < 0},
    ]
    promote = all(c["pass"] for c in checks if c["id"] != "timing_not_closed")
    verdict = "STA_T2_SANDWICH_GP1_BLADES_SPLIT_PASS" if promote else "STA_T2_SANDWICH_GP1_BLADES_SPLIT_FAIL"

    receipt: dict[str, Any] = {
        "schema": "CHIP_CLIFFORD_STA_T2_SANDWICH_GP1_BLADES_SPLIT_RECEIPT_v1",
        ...
        "timing": {
            "wns_ns": wns,
            "prior_t2_21_wns_ns": prior,
            "delta_vs_t2_21_ns": delta_prior,
            "period_ns": 10.0,
        },
        "sim": {"sandwich_dual": sim_dual},
        "opensta": {k: sta.get(k) for k in ("opensta_run", "checks_ok", "wns_ns")},
        "note": "gp2 ara blades intentionally coarse — T2.11 falsifier",
    }
    if write:
...
if __name__ == "__main__":
    print(json.dumps(run_sta_t2_sandwich_gp1_blades_split(), indent=2))
