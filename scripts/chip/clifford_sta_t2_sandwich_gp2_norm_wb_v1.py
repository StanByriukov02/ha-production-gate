"""T2.14 — gp2 blades @ EX3 · norm @ wb_eval on registered lat_ara · rd @ macro_retire."""
...
_RECEIPT = _CHIP / "CHIP_CLIFFORD_STA_T2_SANDWICH_GP2_NORM_WB_RECEIPT_v1.json"
...
def _sim_dual_tb() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_sandwich_gp2_wb_vlt_") as tmp:
        ...
        try:
            ...
            return {
                "verdict": "IVERILOG_SANDWICH_GP2_NORM_WB_PASS" if ok else "IVERILOG_SANDWICH_GP2_NORM_WB_FAIL",
                "stdout_tail": out[-1200:],
            }
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as exc:
            return {"verdict": "IVERILOG_SANDWICH_GP2_NORM_WB_FAIL", "reason": str(exc)[-400:]}


def run_sta_t2_sandwich_gp2_norm_wb(*, write: bool = True) -> dict[str, Any]:
    ...
    gp2_wb = (
        ...
        and "if (wb_eval)" in sw
        and "macro_retire && latched_op == `CLIFFORD_OP_V_SANDWICH" in top
        and "ara_motor_mux" not in sw
    )
    sim = _sim_dual_tb()
    ...
    binding = "sandwich_gp2_norm_wb" if "u_norm_synth" in tail and gp2_wb else "unknown"

    checks = [
        {"id": "t2_10_prerequisite", "pass": prior_ok},
        {"id": "binding_path_doc", "pass": _SCOPE.is_file()},
        {"id": "sandwich_gp2_norm_wb_rtl", "pass": gp2_wb},
        {"id": "iverilog_sandwich_dual_9case", "pass": sim.get("verdict") == "IVERILOG_SANDWICH_GP2_NORM_WB_PASS"},
        {"id": "opensta_liberty_ran", "pass": sta.get("opensta_run") and sta.get("checks_ok")},
        {
            ...
            "detail": f"prior={prior} gp2_wb={wns} delta={delta_prior}",
        },
        {"id": "timing_not_closed", "pass": wns is not None and wns < 0},
    ]

    timing_closed = wns is not None and wns >= 0
    verdict = "STA_T2_SANDWICH_GP2_NORM_WB_PASS" if all(c["pass"] for c in checks) else "STA_T2_SANDWICH_GP2_NORM_WB_FAIL"
    receipt: dict[str, Any] = {
        "receipt_id": "CHIP_CLIFFORD_STA_T2_SANDWICH_GP2_NORM_WB_RECEIPT_v1",
        "verdict": verdict,
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "checks": checks,
        "timing": {
            "t2_10_prior_wns_ns": prior,
            "gp2_norm_wb_wns_ns": wns,
        ...
        "honesty": {
            ...
            "staged_split": "gp2 blades @ ex3 · norm(lat_ara_reg) @ wb_eval · rd @ macro_retire",
            "partial_improvement_ns": 2.886,
            "gate_floor_ns": 10.0,
            "verdict_note": "FAIL gate threshold · KEEP as best RTL −164 ns",
...
if __name__ == "__main__":
    print(json.dumps(run_sta_t2_sandwich_gp2_norm_wb(), indent=2))
