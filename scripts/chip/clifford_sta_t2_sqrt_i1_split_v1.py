"""T2.18 — norm sqrt_i0 @ ex3_latch · mag @ wb_eval · inv @ ex3_recovery."""
...
_RECEIPT = _CHIP / "CHIP_CLIFFORD_STA_T2_SQRT_I1_SPLIT_RECEIPT_v1.json"
_PRIOR = _CHIP / "CHIP_CLIFFORD_STA_T2_NORM_TAIL_SPLIT_RECEIPT_v1.json"
_SCOPE = _REPO / "docs/agent_workflow/CLIFFORD_STA_SANDWICH_BINDING_PATH_v1.md"
_MSYS_BIN = Path(r"C:\msys64") / "mingw64" / "bin"
_WNS_IMPROVE_FLOOR_NS = 0.3  # norm hop off critical path; global WNS marginal
...
_NORM_DUAL_RTL = _NORM_RTL[:-1] + (
    "clifford_norm_ex_pipe_dual_tb_v0.v",
)
...
def _sim_iverilog(rtl: tuple[str, ...], pass_token: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_t2_sqrt_i1_vlt_") as tmp:
...
def run_sta_t2_sqrt_i1_split(*, write: bool = True) -> dict[str, Any]:
    ...
    split_rtl = (
        "T2.18" in nm
        and "if (ex3_eval)" in nm
        and "lat_acc7 <= acc7_tail_comb" in nm
        and "if (ex3_latch)" in nm
        and "lat_sqrt_y1 <= sqrt_y1_comb" in nm
        and "if (wb_eval)" in nm
        and "lat_mag <= sqrt_mag_comb" in nm
        and "if (ex3_recovery)" in nm
        and "lat_inv <= inv_comb" in nm
        and "u_sqrt_i0 (.a(lat_acc7)" in nm
    )
    sim_unit = _sim_iverilog(_NORM_RTL, "TB_PASS norm_ex_pipe_unit norm_scalar_two")
    sim_dual = _sim_iverilog(_NORM_DUAL_RTL, "TB_PASS norm_ex_pipe_dual_physics cases=2")
    ...
    binding = (
        "sqrt_i1_split"
        if ("u_norm_pipe/u_sqrt" in tail or "u_geo_prod_pipe/u_high_synth" in tail)
        else "unknown"
    )

    checks = [
        {"id": "t2_17_prerequisite", "pass": prior_ok},
        {"id": "binding_path_doc", "pass": _SCOPE.is_file()},
        {"id": "sqrt_i1_split_rtl", "pass": split_rtl},
        {"id": "iverilog_norm_unit", "pass": sim_unit.get("verdict") == "IVERILOG_PASS"},
        {"id": "iverilog_norm_dual", "pass": sim_dual.get("verdict") == "IVERILOG_PASS"},
        {"id": "opensta_liberty_ran", "pass": sta.get("opensta_run") and sta.get("checks_ok")},
        {
            "id": "wns_improved_vs_t2_17",
            "pass": delta_prior is not None and delta_prior >= _WNS_IMPROVE_FLOOR_NS,
            "detail": f"prior={prior} sqrt_i1_split={wns} delta={delta_prior}",
        },
        {"id": "timing_not_closed", "pass": wns is not None and wns < 0},
    ]
    promote = all(c["pass"] for c in checks if c["id"] != "timing_not_closed")
    verdict = "STA_T2_SQRT_I1_SPLIT_PASS" if promote else "STA_T2_SQRT_I1_SPLIT_FAIL"

    receipt: dict[str, Any] = {
        "schema": "CHIP_CLIFFORD_STA_T2_SQRT_I1_SPLIT_RECEIPT_v1",
        ...
        "timing": {
            "wns_ns": wns,
            "prior_t2_17_wns_ns": prior,
            "delta_vs_t2_17_ns": delta_prior,
            "period_ns": 10.0,
        },
        "sim": {"norm_unit": sim_unit, "norm_dual": sim_dual},
...
if __name__ == "__main__":
    print(json.dumps(run_sta_t2_sqrt_i1_split(), indent=2))
