"""T2.13 — sandwich norm wb_eval slip: comb norm(lat_ara) · rd @ φ6 wb_eval."""
...
_RECEIPT = _CHIP / "CHIP_CLIFFORD_STA_T2_SANDWICH_NORM_WB_SLIP_RECEIPT_v1.json"
...
def _sim_dual_tb() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_sandwich_wb_vlt_") as tmp:
        ...
        try:
            ...
            return {
                "verdict": "IVERILOG_SANDWICH_NORM_WB_SLIP_PASS" if ok else "IVERILOG_SANDWICH_NORM_WB_SLIP_FAIL",
                "stdout_tail": out[-1200:],
            }
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as exc:
            return {"verdict": "IVERILOG_SANDWICH_NORM_WB_SLIP_FAIL", "reason": str(exc)[-400:]}


def run_sta_t2_sandwich_norm_wb_slip(*, write: bool = True) -> dict[str, Any]:
    ...
    wb_slip = (
        "assign r = norm_mux" in sw
        and "pipe_r" not in sw
        and "u_norm_synth" in sw
        and "T2.13" in sw
    )
    sim = _sim_dual_tb()
    ...
    binding = "sandwich_norm_wb_slip" if "u_norm_synth" in tail and wb_slip else "unknown"

    checks = [
        {"id": "t2_10_prerequisite", "pass": prior_ok},
        {"id": "binding_path_doc", "pass": _SCOPE.is_file()},
        {"id": "sandwich_norm_wb_slip_rtl", "pass": wb_slip},
        {"id": "iverilog_sandwich_dual_9case", "pass": sim.get("verdict") == "IVERILOG_SANDWICH_NORM_WB_SLIP_PASS"},
        {"id": "opensta_liberty_ran", "pass": sta.get("opensta_run") and sta.get("checks_ok")},
        {
            ...
            "detail": f"prior={prior} wb_slip={wns} delta={delta_prior}",
        },
        {"id": "timing_not_closed", "pass": wns is not None and wns < 0},
    ]

    timing_closed = wns is not None and wns >= 0
    verdict = "STA_T2_SANDWICH_NORM_WB_SLIP_PASS" if all(c["pass"] for c in checks) else "STA_T2_SANDWICH_NORM_WB_SLIP_FAIL"
    receipt: dict[str, Any] = {
        "receipt_id": "CHIP_CLIFFORD_STA_T2_SANDWICH_NORM_WB_SLIP_RECEIPT_v1",
        "verdict": verdict,
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "checks": checks,
        "timing": {
            "t2_10_prior_wns_ns": prior,
            "norm_wb_slip_wns_ns": wns,
        ...
        "honesty": {
            ...
            "macro_cycle_slip": "norm comb r · rd @ wb_eval — REVERTED",
            "t2_13_falsifier": "OpenSTA still binds norm cloud; MCP slip did not cut comb depth",
            "rtl_reverted_to": "T2.10 pipe_r @ ex3_latch",
...
if __name__ == "__main__":
    print(json.dumps(run_sta_t2_sandwich_norm_wb_slip(), indent=2))
