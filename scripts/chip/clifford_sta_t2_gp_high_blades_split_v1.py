"""T2.20 — geo_prod high blade internal split: grades 4+5 @ ex3_eval · 6+7 @ ex3_latch."""
...
_RECEIPT = _CHIP / "CHIP_CLIFFORD_STA_T2_GP_HIGH_BLADES_SPLIT_RECEIPT_v1.json"
_PRIOR = _CHIP / "CHIP_CLIFFORD_STA_T2_GP_HIGH_STAGED_RECEIPT_v1.json"
...
def _sim_iverilog(rtl: tuple[str, ...], pass_token: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="clifford_t2_gp_hi_split_vlt_") as tmp:
...
def run_sta_t2_gp_high_blades_split(*, write: bool = True) -> dict[str, Any]:
    ...
    staged_rtl = (
        "T2.20" in gp
        and "lat_high_lo" in gp
        and "u_high_lo_synth" in gp
        and "u_high_hi_synth" in gp
        and "if (ex3_eval)" in gp
        and "lat_high_lo <= comb_high_lo_mux" in gp
        and "if (ex3_latch)" in gp
        and "lat_high <= {comb_high_hi_mux, lat_high_lo}" in gp
        and ".ex3_eval(ex3_eval)" in top
    )
    ...
    binding = (
        "gp_high_hi_blades"
        if "u_high_hi_synth" in tail
        else ("gp_high_lo_blades" if "u_high_lo_synth" in tail else "unknown")
    )

    checks = [
        {"id": "t2_19_prerequisite", "pass": prior_ok},
        {"id": "binding_path_doc", "pass": _SCOPE.is_file()},
        {"id": "gp_high_blades_split_rtl", "pass": staged_rtl},
        ...
        {
            "id": "wns_improved_vs_t2_19",
            "pass": delta_prior is not None and delta_prior >= _WNS_IMPROVE_FLOOR_NS,
            "detail": f"prior={prior} gp_hi_split={wns} delta={delta_prior}",
        },
        {"id": "timing_not_closed", "pass": wns is not None and wns < 0},
    ]
    promote = all(c["pass"] for c in checks if c["id"] != "timing_not_closed")
    verdict = "STA_T2_GP_HIGH_BLADES_SPLIT_PASS" if promote else "STA_T2_GP_HIGH_BLADES_SPLIT_FAIL"

    receipt: dict[str, Any] = {
        "schema": "CHIP_CLIFFORD_STA_T2_GP_HIGH_BLADES_SPLIT_RECEIPT_v1",
        ...
        "timing": {
            "wns_ns": wns,
            "prior_t2_19_wns_ns": prior,
            "delta_vs_t2_19_ns": delta_prior,
            "period_ns": 10.0,
        },
...
if __name__ == "__main__":
    print(json.dumps(run_sta_t2_gp_high_blades_split(), indent=2))
